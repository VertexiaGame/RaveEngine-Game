...
const GROUNDED_CHECK_INTERVAL: std::time::Duration = std::time::Duration::from_millis(33);

#[derive(Component)]
pub struct PlayerInputTracker {
    pub last_input: Instant,
    pub last_grounded_check: Instant,
}
...
impl Default for PlayerInputTracker {
    fn default() -> Self {
        Self {
            ...
            last_grounded_check: Instant::now(),
        }
    }
}
...
pub fn handle_player_inputs(
    ...
    for (client_entity, remote_id, mut receiver) in receivers.iter_mut() {
        ...
        let Ok((player, mut transform, mut lin_vel, colliding, mut tracker)) =
        ...
        let mut is_grounded = !colliding.is_empty() && lin_vel.y.abs() < 0.2;
        if !is_grounded
            && lin_vel.y <= 0.0
            && now.duration_since(tracker.last_grounded_check) >= GROUNDED_CHECK_INTERVAL
        {
            tracker.last_grounded_check = now;
...
pub fn zero_stale_player_velocities(
    mut players: Query<(&mut LinearVelocity, &ControlledBy, &PlayerInputTracker)>,
) {
    let now = Instant::now();
    for (mut lin_vel, _controlled_by, tracker) in &mut players {
        if now.duration_since(tracker.last_input) > INPUT_TIMEOUT {
            if lin_vel.x != 0.0 || lin_vel.z != 0.0 {
                lin_vel.x = 0.0;
                lin_vel.z = 0.0;
            }
        }
